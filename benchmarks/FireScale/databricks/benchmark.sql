    ...
    FROM (
      ...
      FROM (
        ...
        FROM (
          SELECT
            CASE
              WHEN 'Date' = 'Date' THEN to_char(uv.visitdate, 'yyyy-MM-dd')::VARCHAR(10)
              WHEN 'Date' = 'Week' THEN to_char(date_trunc('week', uv.visitdate), 'yyyy-MM-dd')::VARCHAR(10)
              WHEN 'Date' = 'Month' THEN to_char(date_trunc('month', uv.visitdate), 'yyyy-MM')::VARCHAR(7)
